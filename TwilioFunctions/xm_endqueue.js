...
exports.handler = function (context, event, callback) {
  ...
  if (!event.Digits) {
    const gather = twiml.gather({
      ...
      action:
        ...
        "?setting=" +
        encodeURI(JSON.stringify(settings) + "&group=" + event.group),
    });
    ...
    gather.say(
      ...
      "None of the on-call agents in the " +
        event.group +
        " group answered. Press 1, to alert the team they missed your call via sms and email. Press 2, to return to the main menu. Hang-up to try again later."
    );
  ...
  //Create xMatters alert
    ...
    got
      ...
      .then(function (response) {
        twiml.say(
          { voice: settings.voice },
          "An x matters alert has been sent to the " +
            event.group +
            " group. They should reach out to you soon."
        );
        twiml.hangup();
  ...
  // Return to main menu
    ...
    twiml.redirect(
      "https://" + context.DOMAIN_NAME + settings.xm_settings + "?Digits=*"
    );
