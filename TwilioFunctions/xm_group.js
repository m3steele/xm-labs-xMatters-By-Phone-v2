exports.handler = function (context, event, callback) {
  ...
  if (
    (event.Digits !== undefined && settings.alertTypes.length >= parseInt(event.Digits, 10)) ||
  ...
  // Incorrect Digits Repeat last step
    ...
    const gather = twiml.gather({
      ...
      numDigits: 1,
      timeout: 10,
      action: 'https://' + context.DOMAIN_NAME + settings.xm_group + '?setting=' + encodeURI(JSON.stringify(settings)),
    });
    gather.say(
    ...
    twiml.redirect('https://' + context.DOMAIN_NAME + settings.xm_group + '?setting=' + encodeURI(JSON.stringify(settings)));
