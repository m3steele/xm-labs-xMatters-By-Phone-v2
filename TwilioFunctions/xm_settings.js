...
exports.handler = function (context, event, callback) {
  ...
  */
  ...
  setting.xmattersHTTP =
    "https://company.cs1.xmatters.com/api/integration/1/functions/e6deb630-5172/triggers?apiKey=91643be0-b878";
  ...
  if (setting.NumberofGroups === 1) {
    setting.Alert_Phrase =
      "Sure, I'll send an Alert to " + setting.Speak_Groups[0] + " group. ";
    setting.Conference_Phrase =
      "Sure, I'll start a conference bridge and invite " +
      setting.Speak_Groups[0] +
      " group. ";
    setting.Incident_Phrase =
      "Sure, I'll create an xMatters incident targeting " +
      setting.Speak_Groups[0] +
      " group. ";
    setting.Livecall_Phrase =
      "Sure, I'll connect you to speak with the primary on call for " +
      setting.Speak_Groups[0] +
      " group. ";
  }
  ...
  for (i = 0; i < setting.NumberofGroups; i++) {
    ...
    digit = i + 1;
    setting.Group_Speak_Text +=
      "Press " + digit + " for " + setting.Speak_Groups[i] + ". ";
  }
  ...
  if (setting.alertTypes.includes("Alert")) {
    ...
    alert.m =
      "Press " +
      parseInt(setting.alertTypes.indexOf("Alert") + 1) +
      " to alert an x matters group.";
  } else {
    alert.d = "";
    alert.m = "";
  }
  ...
  if (setting.alertTypes.includes("Conference")) {
    ...
    conference.m =
      "Press " +
      parseInt(setting.alertTypes.indexOf("Conference") + 1) +
      " to create an x  matters conference.";
  } else {
    conference.d = "";
    conference.m = "";
  }
  ...
  if (setting.alertTypes.includes("Incident")) {
    ...
    incident.m =
      "Press " +
      parseInt(setting.alertTypes.indexOf("Incident") + 1) +
      " to create an x  matters incident.";
  ...
  if (setting.alertTypes.includes("Livecall")) {
    ...
    livecall.m =
      "Press " +
      parseInt(setting.alertTypes.indexOf("Livecall") + 1) +
      " to speak directly with the primary on call.";
  } else {
    livecall.d = "";
    livecall.m = "";
  }
      ...
      if (results.indexOf("Yes") > -1) {
        ...
        twiml.redirect(
          ...
          "https://" +
            ...
            setting.xm_action +
            "?setting=" +
            encodeURI(JSON.stringify(setting))
        );
  ...
  function get_xm_devices(user) {
    ...
    return got(setting.xmatters + "/api/xm/1/people/" + user + "/devices", {
      headers: {
        ...
        Authorization:
          "Basic " +
          Buffer.from(setting.xm_user + ":" + setting.xm_pass).toString(
            "base64"
          ),
      },
      json: true,
      ...
      .then(function (response) {
        ...
        for (var device in resdata) {
          ...
          if (resdata[device].deviceType === "VOICE") {
            console.log(
              "Get VOICE Device: '" +
                JSON.stringify(resdata[device].phoneNumber) +
                " Initiating Phone: " +
